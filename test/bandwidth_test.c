...
//   if(microtcp_bind(&socket, (struct sockaddr*) &address, sizeof(address)) == -1) {
//     perror("microtcp_bind");
//     return -EXIT_FAILURE;
//   }
//   microtcp_accept(&socket, (struct sockaddr*) &address, sizeof(address));
//   char buffer[MICROTCP_RECVBUF_LEN];
//   clock_gettime (CLOCK_MONOTONIC_RAW, &start_time);
...
//   print_statistics (socket.bytes_received, start_time, end_time);
//   return 0;
// }
int
server_microtcp (uint16_t listen_port, const char *file)
...
//   microtcp_send(&socket, buffer, fileSize, 0);
//   fclose(fp);
//   return 0;
// }
